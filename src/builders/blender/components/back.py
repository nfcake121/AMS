...
def _build_back_from_spec(plan, spec: BackSpec, ctx: BuildContext, helpers: BackBuildHelpers) -> BackBuildResult:
    ...
    if not has_back_support:
        emit_simple(
            ...
            source="computed",
            payload={
                "key": {"mode": "legacy", "orientation": None, "layout": None},
                "handler": "legacy_back_frame",
            },
            reason="dispatch back build strategy",
        )
        ...
        dispatch_key = (dispatch_mode, dispatch_orientation, dispatch_layout)
        dispatch_map = {
            ("panel", None, None): "back_panel",
            ("straps", None, None): "back_straps",
            ("slats", "horizontal", "full"): "back_slats",
            ("slats", "horizontal", "split_center"): "back_slats",
            ("slats", "vertical", "full"): "back_slats",
            ("slats", "vertical", "split_center"): "back_slats",
            ("noop", None, None): "back_noop",
        }
        handler_name = dispatch_map[dispatch_key]
        emit_simple(
            ctx.diag,
            run_id=ctx.run_id,
            stage="build",
            ...
            payload={
                "key": {
                    "mode": dispatch_mode,
                    "orientation": dispatch_orientation,
        ...
        if handler_name == "back_panel":
            build_back_panel_strategy(
                plan=plan,
                seat_total_width_mm=seat_total_width_mm,
                back_frame_member_mm=back_frame_member_mm,
                back_frame_height_mm=back_frame_height_mm,
                back_panel_center=back_panel_center,
            )
        elif handler_name == "back_straps":
            build_back_straps_strategy(
                plan=plan,
                back_strap_count=back_strap_count,
                back_strap_width_mm=back_strap_width_mm,
                back_strap_thickness_mm=back_strap_thickness_mm,
                back_frame_height_mm=back_frame_height_mm,
                back_frame_member_mm=back_frame_member_mm,
                back_margin_z_mm=back_margin_z_mm,
                seat_total_width_mm=seat_total_width_mm,
                back_frame_base_z=back_frame_base_z,
                back_frame_center_y=back_frame_center_y,
            )
        elif handler_name == "back_slats":
            back_slats_bbox_inner_text = build_back_slats_strategy(
                plan=plan,
                ...
                back_slat_orientation=back_slat_orientation,
                back_slat_layout=back_slat_layout,
                has_back_slat_gap=has_back_slat_gap,
                back_slat_gap_mm=back_slat_gap_mm,
                back_slat_center_gap_mm=back_slat_center_gap_mm,
                back_rail_inset_mm=back_rail_inset_mm,
                back_margin_x_mm=back_margin_x_mm,
                back_margin_z_mm=back_margin_z_mm,
                back_rail_width_mm=back_rail_width_mm,
                back_rail_depth_mm=back_rail_depth_mm,
                back_rail_height_mm=back_rail_height_mm,
                ...
                rail_left_x=rail_left_x,
                rail_right_x=rail_right_x,
                back_frame_base_z=back_frame_base_z,
                back_frame_top_z=back_frame_top_z,
                back_frame_center_y=back_frame_center_y,
                back_frame_debug_primitives=back_frame_debug_primitives,
                back_slat_debug_primitives=back_slat_debug_primitives,
                build_back_frame_split2_fn=build_back_frame_split2,
                build_back_frame_full_fn=build_back_frame_full,
                build_back_horizontal_slats_fn=build_back_horizontal_slats,
                build_back_vertical_slats_fn=build_back_vertical_slats,
            )
...
def _append_corner_back_return_segment(
    plan,
    *,
    temp_plan: BuildPlan,
    name_prefix: str,
    ...
    local_back_center_y: float,
    chaise_side: str,
) -> None:
    side_sign = 1.0 if chaise_side == "right" else -1.0
    skip_names = {"back_rail_center"}
    for primitive in temp_plan.primitives:
        if primitive.name in skip_names:
            continue
        local_x = float(primitive.location_mm[0])
        local_y = float(primitive.location_mm[1])
        local_z = float(primitive.location_mm[2])
        delta_y = local_y - float(local_back_center_y)
        new_x = float(return_center_x) + (side_sign * delta_y)
        new_y = float(return_center_y) + local_x
        dim_x, dim_y, dim_z = primitive.dimensions_mm
        plan.primitives.append(
            Primitive(
                name=_rename_corner_back_name(name_prefix, primitive.name),
                shape=primitive.shape,
                dimensions_mm=(float(dim_y), float(dim_x), float(dim_z)),
                location_mm=(new_x, new_y, local_z),
    ...
    for anchor in temp_plan.anchors:
        if anchor.name in skip_names:
            continue
        local_x = float(anchor.location_mm[0])
        local_y = float(anchor.location_mm[1])
        local_z = float(anchor.location_mm[2])
        delta_y = local_y - float(local_back_center_y)
        new_x = float(return_center_x) + (side_sign * delta_y)
        new_y = float(return_center_y) + local_x
        plan.anchors.append(
...
def _build_corner_back_return_segment(
    *,
    plan,
    inputs: BackInputs,
    ctx: BuildContext,
    ...
    if not bool(chaise_request.allowed):
        _emit_slot_skipped(
            ctx,
        ...
        return False
    return_length_mm = max(1.0, float(chaise_request.max_y) - float(chaise_request.min_y))
    chaise_side = _detect_chaise_side(main_request, chaise_request)
    helpers = BackBuildHelpers(
        seat_total_width_mm=return_length_mm,
        total_width_mm=return_length_mm,
        seat_depth_mm=float(inputs.seat_depth_mm),
        frame_thickness_mm=float(inputs.frame_thickness_mm),
        seat_support_top_z=float(inputs.seat_support_top_z),
        base_frame_top_z=float(inputs.base_frame_top_z),
        base_frame_center_z=float(inputs.base_frame_center_z),
        back_y=float(chaise_request.back_plane_y),
    )
    temp_plan = BuildPlan(metadata={})
    back_result = _build_back_from_spec(plan=temp_plan, spec=inputs.back, ctx=ctx, helpers=helpers)
    _append_back_zone_anchors(plan=temp_plan, back_result=back_result, helpers=helpers)
    rail_depth_mm = max(1.0, float(inputs.back.frame.rail_depth_mm))
    if chaise_side == "right":
        return_center_x = float(chaise_request.min_x) + (rail_depth_mm / 2.0)
    else:
        return_center_x = float(chaise_request.max_x) - (rail_depth_mm / 2.0)
    return_center_y = (float(chaise_request.min_y) + float(chaise_request.max_y)) / 2.0
    _append_corner_back_return_segment(
        plan,
        temp_plan=temp_plan,
        name_prefix="return",
        return_center_x=return_center_x,
        return_center_y=return_center_y,
        local_back_center_y=back_result.back_panel_center[1],
    ...
    if inputs.layout_kind == "corner" and inputs.requests:
        ...
        if main_request is not None and chaise_request is not None:
            ...
            if is_collinear:
                ...
                if _build_corner_back_segment(
                    ...
                    built_main_continuous = True
            if built_main_continuous:
                built_return = _build_corner_back_return_segment(
                    plan=plan,
                    inputs=inputs,
                    ctx=ctx,
                ...
                if built_return:
                    slots_used.append("back_return")
        elif main_request is not None:
            if _build_corner_back_segment(
                plan=plan,
                inputs=inputs,
                ctx=ctx,
                ...
                slots_used.append("main_back")
                built_main_continuous = True
        elif chaise_request is not None:
            if _build_corner_back_segment(
                plan=plan,
                inputs=inputs,
                ctx=ctx,
        ...
        emit_simple(
            ...
            payload={
                ...
                "split_mode": "merged+return" if built_return else "single_segment",
            },
